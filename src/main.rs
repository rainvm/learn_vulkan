...
use vulkanalia::vk::{
    ExtDebugUtilsExtension, KhrSurfaceExtension, KhrSwapchainExtension, PhysicalDeviceType,
};
...
unsafe fn create_instance(window: &Window, entry: &Entry, data: &mut AppData) -> Result<Instance> {
    ...
    let application_info = vk::ApplicationInfo::builder()
        .application_name(b"Hello Vulkan")
...
unsafe fn create_pipeline(device: &Device, data: &mut AppData) -> Result<()> {
    ...
    let dynamic_states = vk::PipelineDynamicStateCreateInfo::builder().dynamic_states(dynamic_states);
...
unsafe fn create_buffer(
    ...
    //TODO Don't allocate memory for every buffer. Create custom allocator that allocates for multiple objects using offset. Staging Buffer page
...
unsafe fn create_texture_image(instance: &Instance, device: &Device, data: &mut AppData) -> Result<()>{
    let image = File::open("assets/resources/statue.png")?;

    let decoder = png::Decoder::new(image);
    let mut reader = decoder.read_info()?;

    let mut pixels = vec![0; (reader.info().width * reader.info().height) as usize];
    reader.next_frame(&mut pixels)?;

    let size = (reader.info().height * reader.info().width) as u64;
